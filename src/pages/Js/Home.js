document.addEventListener("DOMContentLoaded", function() {
    ...
    // Hide language options when clicking outside the menu
    document.addEventListener("click", function(event) {
        if (!selector.contains(event.target) && !options.contains(event.target) && !arrowDown.contains(event.target)) {
            options.classList.remove("show");
        }
    });
    ...
    if (savedLanguageFlag && savedLanguage) {
        selectedLanguageFlag.innerHTML = savedLanguageFlag;
